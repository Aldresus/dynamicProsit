...
describe('prosit form filling', () => {
  ...
  it('fill prosit form', () => {
    ...
    cy.get("input[name=generalisation]").should("have.value", prositData.generalisation)
    cy.get("textarea[name=contexte]").should("have.value", prositData.contexte)

    cy.get("input[name=animateur]").should("have.value", prositData.animateur)
    cy.get("input[name=scribe]").should("have.value", prositData.scribe)
    cy.get("input[name=gestionnaire]").should("have.value", prositData.gestionnaire)
    cy.get("input[name=secretaire]").should("have.value", prositData.secretaire)
    ...
    cy.get("#editable-item-list-motsCles").each((item, index) => {
      //the child contains a h3 and a button
        cy.wrap(item).children().eq(0).children().eq(0).should("have.text","-"+
            prositData.motsCles[index])
    } )
    ...
    cy.get("#editable-item-list-problematiques").each((item, index) => {
      //the child contains a h3 and a button
        cy.wrap(item).children().eq(0).children().eq(0).should("have.text","-"+
            prositData.problematiques[index])
    } )
    ...
    cy.get("#editable-item-list-pistesDeSolutions").each((item, index) => {
      //the child contains a h3 and a button
        cy.wrap(item).children().eq(0).children().eq(0).should("have.text","-"+
            prositData.pistesDeSolutions[index])
    } )
    ...
    cy.get("#editable-item-list-plan-d-action").each((item, index) => {
      //the child contains a h3 and a button
        cy.wrap(item).children().eq(0).children().eq(0).should("have.text",(index+1)+
            prositData.planDAction[index].content)
    } )
    ...
    context("presentation mode read the values", () => {
      ...
      cy.get("#titreProsit").should("have.text", "Prosit : "+prositData.titre)
      cy.get("#texte-contexte").should("have.text", prositData.contexte)
      cy.get("#texte-generalisation").should("have.text", prositData.generalisation)


      cy.get("#liste-motsClefs").each((item, index) => {
        //the child contains a h3 and a button
        cy.wrap(item).children().children().children().should("have.text","-"+
            prositData.planDAction[index].content)
      } )
    })
  })
